# Запаси

PWA для обліку домашніх запасів. React + Supabase, хоститься на GitHub Pages.

## Локальний запуск

```
npm install
cp .env.example .env    # вписати anon key
npm run dev
```

## Тести

```
npm test
```

Інтеграційні тести проти реальної бази вимагають `TEST_EMAIL` і `TEST_PASSWORD`
у `.env` — без них вони пропускаються:

```
node --env-file=.env node_modules/vitest/vitest.mjs run tests/integration
```

## Міграції

SQL із `supabase/migrations/` виконується вручну в Supabase SQL Editor
по порядку номерів. Бакет `photos` створюється в панелі Storage **приватним**,
перед застосуванням `0005_storage.sql`.

## Вхід

Пошта і пароль. Magic link лишився запасним варіантом: вбудована пошта
Supabase жорстко обмежена по кількості листів на годину.

Змінити пароль і вийти — у розділі «Ще» всередині застосунку.

## Безпека

`anon` ключ публічний за задумом — він вшивається у збірку і видимий кожному
відвідувачу сайту незалежно від того, публічний репозиторій чи ні. Дані
захищає не таємність ключа, а RLS і логін.

- `service_role` ключ не використовується ніде. Він обходить RLS повністю.
- Після першого успішного входу вимкнути реєстрацію нових користувачів:
  Supabase -> Authentication -> налаштування Email-провайдера. Інакше будь-хто
  з публічним ключем може створити акаунт у проєкті й витрачати безкоштовний ліміт.
- Бакет `photos` має лишатись приватним. Посилання на фото підписані й живуть годину.

## Перевірка доступу

```
node --env-file=.env scripts/verify-rls.mjs
```

Має вивести три рядки `ok`. Якщо хоч одна таблиця віддає дані анонімному
клієнту — RLS зламано, деплоїти не можна.

## Іконки

```
node scripts/make-icons.mjs
```

## Документація

- Дизайн: `docs/superpowers/specs/2026-09-23-home-inventory-design.md`
- План: `docs/superpowers/plans/2026-09-23-home-inventory.md`
